...
namespace Common.WebApiCore.Identity
{
        ...
        where TUser : Entities.User, new()
    {
        ...
        public AuthenticationService(JwtManager jwtManager, UserManager<TUser> userManager, IOditRepository oditRepository)
        {
        ...
        public async Task<AuthResult<Token>> Login(LoginDTO loginDto)
        {
            ...
            if (user != null && user.Id > 0)
            {
                if (await userManager.CheckPasswordAsync(user, loginDto.Password))
                {
                    ...
                    var result = AuthResult<Token>.TokenResult(token);
                    if (result.Succeeded)
                    {
                        var row = oditRepository.AddRow(1, "", user.Id.ToString());
                    }
                    return result;
                }
            }
        ...
        public async Task<AuthResult<Token>> SignUp(SignUpDTO signUpDto)
        {
            ...
            var newUser = new TUser { Login = signUpDto.FullName, Email = signUpDto.Email.ToUpperInvariant() };
