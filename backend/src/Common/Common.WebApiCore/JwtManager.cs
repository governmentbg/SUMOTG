...
namespace Common.WebApiCore
{
    public class JwtManager
    {
        ...
        private ClaimsIdentity GenerateClaimsIdentity<TUser>(TUser user) where TUser : User
        {
            ...
            if (user.UserObhvat != null)
            {
                claims.AddRange(user.UserObhvat.Select(x => new Claim("scope", x.RaionId==null ? "0": x.RaionId)));
            }else
            {
                var sequence = Enumerable.Empty<Claim>();
                sequence.Append <Claim>(new Claim("scope", "0"));
                claims.AddRange(sequence);
            }
