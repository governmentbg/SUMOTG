...
BEGIN
    CREATE TABLE [dbo].[dem_dogovor] (
        [Id_firma_DM] int NOT NULL IDENTITY(100, 1),
        [Faza] smallint NULL,
        [EIK] varchar(14) NULL,
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210911103359_AddedUserObhvatRaionId')
BEGIN
    ALTER TABLE [dbo].[UserObhvat] ADD [RaionId] nvarchar(max) NULL;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20210911103359_AddedUserObhvatRaionId', N'5.0.8');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210914175523_ChangedLicaFieldsSize')
BEGIN
    BEGIN
    DECLARE @db_name NVARCHAR(MAX) = DB_NAME();
    EXEC(N'ALTER DATABASE [' + @db_name + '] COLLATE Cyrillic_General_CI_AS;');
    END
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica]') AND [c].[name] = N'e_mail');
    IF @var61 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica] DROP CONSTRAINT [' + @var61 + '];');
    ALTER TABLE [dbo].[lica] ALTER COLUMN [e_mail] nvarchar(100) NULL;
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica]') AND [c].[name] = N'UL');
    IF @var62 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica] DROP CONSTRAINT [' + @var62 + '];');
    ALTER TABLE [dbo].[lica] ALTER COLUMN [UL] nvarchar(5) NULL;
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica]') AND [c].[name] = N'KV');
    IF @var63 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica] DROP CONSTRAINT [' + @var63 + '];');
    ALTER TABLE [dbo].[lica] ALTER COLUMN [KV] nvarchar(5) NULL;
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica]') AND [c].[name] = N'JK');
    IF @var64 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica] DROP CONSTRAINT [' + @var64 + '];');
    ALTER TABLE [dbo].[lica] ALTER COLUMN [JK] nvarchar(5) NULL;
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica]') AND [c].[name] = N'IME');
    IF @var65 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica] DROP CONSTRAINT [' + @var65 + '];');
    ALTER TABLE [dbo].[lica] ALTER COLUMN [IME] nvarchar(120) NULL;
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica]') AND [c].[name] = N'A_Raion');
    IF @var66 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica] DROP CONSTRAINT [' + @var66 + '];');
    ALTER TABLE [dbo].[lica] ALTER COLUMN [A_Raion] nvarchar(5) NULL;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20210914175523_ChangedLicaFieldsSize', N'5.0.8');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210917204156_AddPrimaryKeyNRaioni')
BEGIN
    DECLARE @var67 sysname;
    SELECT @var67 = [d].[name]
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[n_raioni]') AND [c].[name] = N'Z_Code');
    IF @var67 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[n_raioni] DROP CONSTRAINT [' + @var67 + '];');
    ALTER TABLE [dbo].[n_raioni] DROP COLUMN [Z_Code];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[n_raioni]') AND [c].[name] = N'kod_nmn');
    IF @var68 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[n_raioni] DROP CONSTRAINT [' + @var68 + '];');
    ALTER TABLE [dbo].[n_raioni] DROP COLUMN [kod_nmn];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210917204156_AddPrimaryKeyNRaioni')
BEGIN
    ALTER TABLE [dbo].[n_raioni] ADD CONSTRAINT [PK_n_raioni_nkod] PRIMARY KEY ([NKOD]);
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20210917204156_AddPrimaryKeyNRaioni', N'5.0.8');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210919131745_AddLicaFormulqrOldUredi')
BEGIN
    DECLARE @var69 sysname;
    SELECT @var69 = [d].[name]
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[n_jk]') AND [c].[name] = N'kod_nmn');
    IF @var69 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[n_jk] DROP CONSTRAINT [' + @var69 + '];');
    ALTER TABLE [dbo].[n_jk] DROP COLUMN [kod_nmn];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210919131745_AddLicaFormulqrOldUredi')
BEGIN
    CREATE TABLE [dbo].[lica_formuliar_olduredi] (
        [Id_ured_F] int NOT NULL IDENTITY,
        [Id_formuliar] int NOT NULL,
        [Id_L] int NOT NULL,
        [Id_KT] int NOT NULL,
        [Broi] int NOT NULL,
        [Status_U] smallint NOT NULL,
        [Status] smallint NOT NULL,
        [user] nvarchar(max) NULL,
        [Koga] datetime2(0) NULL,
        CONSTRAINT [PK_olduredi_f_Id_ured_F] PRIMARY KEY ([Id_ured_F]),
        CONSTRAINT [FK_lica_formuliar_olduredi_lica] FOREIGN KEY ([Id_L]) REFERENCES [dbo].[lica] ([ID_L]) ON DELETE CASCADE
    );
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210919131745_AddLicaFormulqrOldUredi')
BEGIN
    CREATE INDEX [IX_lica_formuliar_olduredi_Id_L] ON [dbo].[lica_formuliar_olduredi] ([Id_L]);
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20210919131745_AddLicaFormulqrOldUredi', N'5.0.8');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921185938_AddedLicaTwoColumns')
BEGIN
    DECLARE @var70 sysname;
    SELECT @var70 = [d].[name]
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_formuliar]') AND [c].[name] = N'nV8');
    IF @var70 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_formuliar] DROP CONSTRAINT [' + @var70 + '];');
    ALTER TABLE [dbo].[lica_formuliar] DROP COLUMN [nV8];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921185938_AddedLicaTwoColumns')
BEGIN
    ALTER TABLE [dbo].[lica] ADD [IdTitulqr] int NOT NULL DEFAULT 0;
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921185938_AddedLicaTwoColumns')
BEGIN
    ALTER TABLE [dbo].[lica] ADD [V7] int NOT NULL DEFAULT 0;
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921185938_AddedLicaTwoColumns')
BEGIN
    ALTER TABLE [dbo].[lica] ADD [nV8] int NOT NULL DEFAULT 0;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20210921185938_AddedLicaTwoColumns', N'5.0.8');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921201809_CorrectionTableLicaFirma')
BEGIN
    ALTER TABLE [dbo].[lica_formuliar_firma] DROP CONSTRAINT [FK_lica_formuliar_firma_lica_IdLNavigationIdL];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921201809_CorrectionTableLicaFirma')
BEGIN
    DROP INDEX [IX_lica_formuliar_firma_IdLNavigationIdL] ON [dbo].[lica_formuliar_firma];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_formuliar_firma]') AND [c].[name] = N'IdLNavigationIdL');
    IF @var71 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_formuliar_firma] DROP CONSTRAINT [' + @var71 + '];');
    ALTER TABLE [dbo].[lica_formuliar_firma] DROP COLUMN [IdLNavigationIdL];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_formuliar_firma]') AND [c].[name] = N'UNom');
    IF @var72 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_formuliar_firma] DROP CONSTRAINT [' + @var72 + '];');
    ALTER TABLE [dbo].[lica_formuliar_firma] DROP COLUMN [UNom];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921201809_CorrectionTableLicaFirma')
BEGIN
    CREATE INDEX [IX_lica_formuliar_firma_IdL] ON [dbo].[lica_formuliar_firma] ([IdL]);
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921201809_CorrectionTableLicaFirma')
BEGIN
    ALTER TABLE [dbo].[lica_formuliar_firma] ADD CONSTRAINT [FK_lica_formuliar_firma_lica] FOREIGN KEY ([IdL]) REFERENCES [dbo].[lica] ([ID_L]) ON DELETE CASCADE;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20210921201809_CorrectionTableLicaFirma', N'5.0.8');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921220831_AddTableDogovorOldUredi')
BEGIN
    ALTER TABLE [dbo].[lica_dogovor_uredi] DROP CONSTRAINT [FK_lica_dogovor_uredi_lica_dogovor];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_dogovor_uredi]') AND [c].[name] = N'Vypros');
    IF @var73 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_dogovor_uredi] DROP CONSTRAINT [' + @var73 + '];');
    ALTER TABLE [dbo].[lica_dogovor_uredi] DROP COLUMN [Vypros];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_dogovor_uredi]') AND [c].[name] = N'Id_L');
    IF @var74 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_dogovor_uredi] DROP CONSTRAINT [' + @var74 + '];');
    ALTER TABLE [dbo].[lica_dogovor_uredi] ALTER COLUMN [Id_L] int NOT NULL;
    ALTER TABLE [dbo].[lica_dogovor_uredi] ADD DEFAULT 0 FOR [Id_L];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_dogovor_uredi]') AND [c].[name] = N'Id_KT');
    IF @var75 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_dogovor_uredi] DROP CONSTRAINT [' + @var75 + '];');
    ALTER TABLE [dbo].[lica_dogovor_uredi] ALTER COLUMN [Id_KT] int NOT NULL;
    ALTER TABLE [dbo].[lica_dogovor_uredi] ADD DEFAULT 0 FOR [Id_KT];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921220831_AddTableDogovorOldUredi')
BEGIN
    DROP INDEX [IX_lica_dogovor_uredi_ID_DOG_L] ON [dbo].[lica_dogovor_uredi];
    DECLARE @var76 sysname;
    SELECT @var76 = [d].[name]
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_dogovor_uredi]') AND [c].[name] = N'ID_DOG_L');
    IF @var76 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_dogovor_uredi] DROP CONSTRAINT [' + @var76 + '];');
    ALTER TABLE [dbo].[lica_dogovor_uredi] ALTER COLUMN [ID_DOG_L] int NOT NULL;
    ALTER TABLE [dbo].[lica_dogovor_uredi] ADD DEFAULT 0 FOR [ID_DOG_L];
    CREATE INDEX [IX_lica_dogovor_uredi_ID_DOG_L] ON [dbo].[lica_dogovor_uredi] ([ID_DOG_L]);
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_dogovor_uredi]') AND [c].[name] = N'Broi');
    IF @var77 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_dogovor_uredi] DROP CONSTRAINT [' + @var77 + '];');
    ALTER TABLE [dbo].[lica_dogovor_uredi] ALTER COLUMN [Broi] int NOT NULL;
    ALTER TABLE [dbo].[lica_dogovor_uredi] ADD DEFAULT 0 FOR [Broi];
END;
GO
...
BEGIN
    CREATE TABLE [dbo].[lica_dogovor_olduredi] (
        ...
        [user] varchar(128) NULL,
        [Koga] datetime2(0) NULL,
        CONSTRAINT [PK_uredi_dg_l_Id_OldUred_DG] PRIMARY KEY ([Id_Ured_DG]),
        CONSTRAINT [FK_lica_dogovor_olduredi_lica_dogovor] FOREIGN KEY ([ID_DOG_L]) REFERENCES [dbo].[lica_dogovor] ([Id_dog_L]) ON DELETE CASCADE
    );
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921220831_AddTableDogovorOldUredi')
BEGIN
    CREATE INDEX [IX_lica_dogovor_olduredi_ID_DOG_L] ON [dbo].[lica_dogovor_olduredi] ([ID_DOG_L]);
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921220831_AddTableDogovorOldUredi')
BEGIN
    ALTER TABLE [dbo].[lica_dogovor_uredi] ADD CONSTRAINT [FK_lica_dogovor_uredi_lica_dogovor] FOREIGN KEY ([ID_DOG_L]) REFERENCES [dbo].[lica_dogovor] ([Id_dog_L]) ON DELETE CASCADE;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20210921220831_AddTableDogovorOldUredi', N'5.0.8');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921221645_KorekciqLicaDogovor')
BEGIN
    ALTER TABLE [dbo].[lica_dogovor] DROP CONSTRAINT [FK_lica_dogovor_lica];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_dogovor]') AND [c].[name] = N'U_Nom');
    IF @var78 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_dogovor] DROP CONSTRAINT [' + @var78 + '];');
    ALTER TABLE [dbo].[lica_dogovor] DROP COLUMN [U_Nom];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921221645_KorekciqLicaDogovor')
BEGIN
    DROP INDEX [IX_lica_dogovor_Id_L] ON [dbo].[lica_dogovor];
    DECLARE @var79 sysname;
    SELECT @var79 = [d].[name]
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_dogovor]') AND [c].[name] = N'Id_L');
    IF @var79 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_dogovor] DROP CONSTRAINT [' + @var79 + '];');
    ALTER TABLE [dbo].[lica_dogovor] ALTER COLUMN [Id_L] int NOT NULL;
    ALTER TABLE [dbo].[lica_dogovor] ADD DEFAULT 0 FOR [Id_L];
    CREATE INDEX [IX_lica_dogovor_Id_L] ON [dbo].[lica_dogovor] ([Id_L]);
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_dogovor]') AND [c].[name] = N'Faza');
    IF @var80 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_dogovor] DROP CONSTRAINT [' + @var80 + '];');
    ALTER TABLE [dbo].[lica_dogovor] ALTER COLUMN [Faza] smallint NOT NULL;
    ALTER TABLE [dbo].[lica_dogovor] ADD DEFAULT CAST(0 AS smallint) FOR [Faza];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210921221645_KorekciqLicaDogovor')
BEGIN
    ALTER TABLE [dbo].[lica_dogovor] ADD CONSTRAINT [FK_lica_dogovor_lica] FOREIGN KEY ([Id_L]) REFERENCES [dbo].[lica] ([ID_L]) ON DELETE CASCADE;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20210921221645_KorekciqLicaDogovor', N'5.0.8');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210922204730_ChnangeLicaDogUrediStatus')
BEGIN
    DECLARE @var81 sysname;
    SELECT @var81 = [d].[name]
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_dogovor_olduredi]') AND [c].[name] = N'Status_U');
    IF @var81 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_dogovor_olduredi] DROP CONSTRAINT [' + @var81 + '];');
    ALTER TABLE [dbo].[lica_dogovor_olduredi] ALTER COLUMN [Status_U] smallint NOT NULL;
    ALTER TABLE [dbo].[lica_dogovor_olduredi] ADD DEFAULT CAST(0 AS smallint) FOR [Status_U];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[lica_dogovor_olduredi]') AND [c].[name] = N'Status');
    IF @var82 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[lica_dogovor_olduredi] DROP CONSTRAINT [' + @var82 + '];');
    ALTER TABLE [dbo].[lica_dogovor_olduredi] ALTER COLUMN [Status] smallint NOT NULL;
    ALTER TABLE [dbo].[lica_dogovor_olduredi] ADD DEFAULT CAST(0 AS smallint) FOR [Status];
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20210922204730_ChnangeLicaDogUrediStatus', N'5.0.8');
END;
GO
