...
export class ExportMontazjExcelService {
  ...
  exportPorychka(excelData) {
    ...
    let header = [
      '','№',
      'Район','Ид. № по ОПОС','ИМЕНА','Уред','Брой','Вид имот','Адрес на монтажа',
      'Електронен адрес (email)','Телефон за контакти',	'Планиран за дата',	'От час','До час','Забележка'	,'Статус График',
      'Статус Монтаж','Монтиран на дата','Модел','Сериен №','Гар. карта №',	'Дата на Гар. карта','Дата на Приемо-пред. протокол', 'Забележка',
      'Забележка 2','Пор. №'
    ];

    const colsizes = [
      0,0,
      5,15,12,20,30,5,15,30,20,12,
      10,7,7,20,10,
      10,10,15,15,15,10,10,20,15,10
    ]
    ...
    //otchet range
    range = {start: {row: 13, col: 17}, end: {row: 15, col: colsizes.length-1}};
    for (let row = range.start.row; row <= range.end.row; row++) {
      for (let col = range.start.col; col <= range.end.col; col++) {    
        let s = worksheet.getCell(row, col)
        s.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
    ...
    for( let i =datarow; i < data.length+datarow; i++) {
      ...
      worksheet.getCell(`P${i}`).dataValidation = {
        type: 'list',
        allowBlank: true,
        formulae: ['"1-За монтаж, 2-За замяна, 3-Ненамерен, 4-Отказан, 5-Отложен"'],
      ...
      worksheet.getCell(`Q${i}`).dataValidation = {
        type: 'list',
        allowBlank: true,
        formulae: ['"1-Монтиран, 2-За замяна, 3-Ненамерен, 4-Отказан, 5-Отложен"'],
        showInputMessage: true,
        showErrorMessage: true,
        errorStyle: 'stop',
        errorTitle: 'Избор на статус',
      ...
      //data na gar.karta
      worksheet.getCell(`V${i}`).numFmt = 'dd.mm.yyyy'; 

      //data na protokola
      worksheet.getCell(`W${i}`).numFmt = 'dd.mm.yyyy'; 
    }
