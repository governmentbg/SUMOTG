...
@Component({
  selector: 'ngx-demoncontract',
  templateUrl: './demoncontract.component.html',
  styleUrls: ['./demoncontract.component.scss']
})
export class DeMonContractComponent implements OnInit, DoCheck  {
  mainForm: FormGroup;
  previousUrl: string = '';
  click: boolean = false;
  ...
  loadLists() {
    ...
    this.nomenclatureService
      ...
      this.vlistv18 = this.vlistv18.filter(x=>x.id !== 22)
    });
  ...
  save() {
    if (!this.mainForm.valid) {
      ...
      if (this.checkControls(item) == 0) {
        ...
        observable = this.firmaService.setDeMonDogovor(item)
  ...
  back() {
    this.router.navigateByUrl('pages/firmcontracts/demontazj');
  }
...
// nqma raioni
  ...
  exportexcel(): void {
    ...
    items.controls.forEach((form: FormGroup) => {  
      ...
      obj = {
          nrow: ++i,
          uredname: this.vlistv18.find(e => e.nkod === form.get('idured').value).name,
    ...
    let reportData = {
      title: 'Задание за демонтаж',
      sheet: 'Задание',
      colsizes: [0,5,60,10,10,10],
      header: ['No','Уред за демонтаж','Брой','Средно тегло (кг)','Общо тегло (кг)'],
      data: dataForExcel,
      dogovor: this.mainForm.get('regnom').value+'/'+this.mainForm.get('regnomdata').value,
      fname: fname,
