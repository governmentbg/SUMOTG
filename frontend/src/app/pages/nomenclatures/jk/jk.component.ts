...
import { NomenclatureData, NomJk, NomKvartal } from '../../../@core/interfaces/common/nomenclatures';
import { Screens } from '../../../@core/tools/screens';
import { EditJkDialogComponent } from './edit-dialog/edit-dialog.component';

@Component({
  selector: 'ngx-jk',
  templateUrl: './jk.component.html',
  styleUrls: ['./jk.component.scss'],
})
export class JkComponent implements OnInit {
  ...
  addItem() {
    ...
    .getMaxKodJk()
    .subscribe(result => {    
        this.dialogService.open(EditJkDialogComponent
                          , { hasBackdrop: true,
  ...
  removeItem($data: NomJk) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.nomenclatureService.delRowNomenJk( $data.nkod).subscribe(r=> {
            if (r) {
