...
export class EditNsMestaDialogComponent implements OnInit {
  id: string = '';
  EditForm!: FormGroup;
  forbiddenCodes = ['00'];
  ...
  constructor(
    protected ref: NbDialogRef<EditNsMestaDialogComponent>,
  ...
  save() {
    ...
    if (this.isnewItem) {
      this.nomenclatureService.addRowNomenNsMesta(obj).subscribe(r=> {
        if (r) {
          if (r <= 0) {
      ...
      this.nomenclatureService.setRowNomenNsMesta(obj).subscribe(r=> {
        if (r) {
          this.ref.close(this.EditForm.value);  
        } else {
          this.handleWrongResponse(-9);
  ...
  convertToFormulqr(value: any): NomNsMqsto {
    const obj: NomNsMqsto = value;
