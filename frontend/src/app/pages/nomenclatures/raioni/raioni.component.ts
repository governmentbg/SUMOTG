...
import { EditRaioniDialogComponent } from './edit-dialog/edit-dialog.component';

@Component({
  selector: 'ngx-raioni',
  templateUrl: './raioni.component.html',
  styleUrls: ['./raioni.component.scss']
})
export class RaioniComponent implements OnInit {
  nomcode: string = '';
  name: string = '';
  ...
  addItem() {
    this.dialogService.open(EditRaioniDialogComponent
                      , { hasBackdrop: true,
  ...
  removeItem($data: NomRaion) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.nomenclatureService.delRowNomenRaioni( $data.nkod).subscribe(r=> {
            if (r) {
