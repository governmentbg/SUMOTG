...
import { DogovorRaioni, DogovorUredi, MonOrder, ObrabotkaData } from '../../../../@core/interfaces/common/obrabotki';
import { HeaderComponent } from '../../../../@theme/components';
import { ExportDemontazjExcelService } from '../../../../@theme/services/export-demontazj-excel.service';
...
@Component({
  selector: 'ngx-demorderedit',
  templateUrl: './demorderedit.component.html',
  styleUrls: ['./demorderedit.component.scss']
})
export class DemordereditComponent implements OnInit {
  ...
  constructor (
    private ete: ExportDemontazjExcelService,
  ...
  loadOrder () {
    this.orderService.getDemonOrder(this.iddog).subscribe((result) => {
      ...
      this.loadDemDogovorFirma (result.idfirma);
      this.loadDemDogovorFirmaUredi (result.iddogovorfirma);
//      this.loadDemDogovorFirmaRaioni (result.iddogovorfirma);
      ...
      this.mainForm.patchValue({
        ...
        idmonporychka: result.idmonporychka ? String(result.idmonporychka) : null,
      });
      ...
      this.orderService
        .getMonOrdersWithoutDemPorychka()
        .subscribe((result) => {
          this.vmonporychki = result
                              .filter(x => x.spm == 0 || x.spm == this.mainForm.get('idmonporychka').value)
                              .map(item => new ViewNom().convertMonOrder(item));
      });        

      if (this.idporychka > 0) {
        this.mainForm.get('idmonporychka').disable();
      }
  ...
  loadDemDogovorFirma (idfirma: number) {
    this.firmiService.loadDeMonDogovorFirma(idfirma).subscribe((result) => {
      ...
      if (this.vmonfirmidogovor.length > 0) {
        ...
        this.loadDemDogovorFirmaUredi(this.vmonfirmidogovor[0].id);
//        this.loadDemDogovorFirmaRaioni(this.vmonfirmidogovor[0].id);
      }
    });  
  }

  loadDemDogovorFirmaUredi (iddogovorfirma: number) {
    this.orderService.getDemonDogovorFirmaUredi(iddogovorfirma).subscribe((result) => {
      this.vdoguredi = result;
      this.calcPorychkaUredi();
    });  
  }

  loadDemDogovorFirmaRaioni (iddogovorfirma: number) {
    this.orderService.getDemonDogovorFirmaRaioni(iddogovorfirma).subscribe((result) => {
  ...
  save() {
    if (this.checkOrder()) {
      ...
      this.mainForm.get('idmonporychka').enable();
      
      const order: MonOrder = this.mainForm.value;
      order.status_pm = (order.status_pm== 0 ? 1 : order.status_pm);
      order.uredi = this.vdoguredi;
 
      if (order.idmonporychka > 0) {
        const txt = this.vmonporychki.find (x => x.id == order.idmonporychka).name; 
        order.note  = (!order.note ? txt : order.note)
      }
      this.click  = !this.click;
      let observable = new Observable<number>();
      observable = this.orderService.setDemonOrder(order);
...
//    this.mainForm.get('status_pm').setValue('2');
    ...
    if (await this.checkImportGrafik(filename) == 0) {
        ...
        this.dataImportGrafik.forEach(x=> {    
          ...
          if (indx > -1) {
            ...
            item.get('dodata').setValue(moment.utc(x.dodata,DATE_FORMAT));
  ...
  onFirmItemSelected(firma: ViewNom) {
    this.loadDemDogovorFirma (firma.id);
  }

  onDogovorItemSelected(dogovorfirma: ViewNom) {
    this.loadDemDogovorFirmaUredi (dogovorfirma.id)
  }
  ...
  async checkImportGrafik(filename: any) {
        ...
        if (chksum.length>0 && chksum.indexOf('Файлът') == -1) {                     
          ...
          if (indx > -1) {
              ...
              if (item.get('status_m').value > 0) {
                ...
                if (item.get('status_g').value == lstatus.substring(0,1)) {
                    counters [3]++;
    ...
    let reportData = {
      title: 'Резултат от обработката на график - демонтаж',
      sheet: 'Справка',
  ...
  async checkImportMontazj (filename: any) {
        ...
        if (chksum.length>0 && chksum.indexOf('Файлът') == -1) {    
          ...
          if (indx > -1) {
            ...
            if (item.get('status_m').value < 2) {
                ...
                let lnote   = row.getCell('S').value ? row.getCell('S').value : '';

                if (item.get('status_g').value > 1) {
                  counters [4]++;
                  obj.status = '';  
              ...
              if (item.get('status_m').value == status.substring(0,1)) {
                  counters [3]++;
                  obj.status = 'Дублиран запис';    
              } else {    
                  counters [5]++;
    ...
    let reportData = {
      title: 'Резултат от обработката на отчет - демонтаж ',
      sheet: 'Справка',
      colsizes: [0,5,15,35,50],
      header: ['№','№ ОПОС','Имена','Резултат'],
      data: dataForExcel,
  ...
  back() {
    this.router.navigateByUrl('pages/obrabotki/demorder');
  }
