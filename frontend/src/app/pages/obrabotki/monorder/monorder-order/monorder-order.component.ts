...
import { MonordereditDialogComponent } from '../monorderedit-dialog/monorderedit-dialog.component';

@Component({
  selector: 'ngx-monorder-order',
  templateUrl: './monorder-order.component.html',
  styleUrls: ['./monorder-order.component.scss']
})
export class MonorderOrderComponent implements OnInit {
  ...
  addPersPersDogUred() {
    ...
    this.dialogService
        .open(MonordereditDialogComponent
                          , { hasBackdrop: true,
  ...
  async addPorychkaItems(items: any) { 
      ...
      if (x.ischeck) {
        ...
        if (canAdd) {
          await this.obrabotkiService
              .getPersonUredi(iddogovorfirma, x.idL)
              .subscribe(result => {
                  result.forEach((t) => {
  ...
  removeItem(item: FormGroup) {
    Swal.fire({
      ...
      text: 'Искате да изтриете избраният ОПОС!',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        ...
        this.obrabotkiService
            .delMonUrediDogovor(idporychkamain, iddogovor)
            .subscribe((result) => {
        });
      }
    });
