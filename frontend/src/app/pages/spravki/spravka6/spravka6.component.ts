...
@Component({
  selector: 'ngx-spravka6',
  templateUrl: './spravka6.component.html',
  styleUrls: ['./spravka6.component.scss']
})
export class Spravka6Component implements OnInit {
  filter: Filter;
  items: Spravka6 [] = [];
  realItems: Spravka6 [] = [];
  ...
  exportexcel(): void {
    ...
    this.realItems.forEach((item: Spravka6) => {                  
      obj = {
        ...
        restordbroi: item.tspbroi-item.ordbroi+item.rembroi,
    ...
    this.ete.exportSpravka6(reportData,1);  
  }

  back() {
    this.router.navigate(['/pages/register/regfilter/15/'+this.filter.descript])
  }
}
