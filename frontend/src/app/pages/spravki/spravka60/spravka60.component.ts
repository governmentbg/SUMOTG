...
import { Spravka50, Spravka60, SpravkiData } from '../../../@core/interfaces/common/spravki';
...
@Component({
  selector: 'ngx-spravka60',
  templateUrl: './spravka60.component.html',
  styleUrls: ['./spravka60.component.scss']
})
export class Spravka60Component implements OnInit {
  filter: FilterA;
  items: Spravka60 [] = [];
  realItems: Spravka60 [] = [];
  ...
  pageSize = 10;
  countItems:number = 0;
  fileName= 'Списък на извършените профилактиките';
  limit: number = 0;
...
//    this.limit = (this.filter.limit ? this.filter.limit : 100);
  ...
  exportexcel(): void {
    ...
    let reportData = {
      ...
      colsizes: [0,15,10,30,30,40,10,5,15,15,30,15,30,15],
      header: [
        ,'ОПОС'  
        ,'№ Поръчка'  
        ,'Уред'
  ...
  back() {
    this.router.navigate(['/pages/register/sprfiltera/5/'+this.filter.descript])
  }
