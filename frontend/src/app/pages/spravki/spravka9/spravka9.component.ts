...
@Component({
  selector: 'ngx-spravka9',
  templateUrl: './spravka9.component.html',
  styleUrls: ['./spravka9.component.scss']
})
export class Spravka9Component implements OnInit {
  filter: Filter;
  items: Spravka5 [] = [];
  realItems: Spravka5 [] = [];
  ...
  exportexcel(): void {
    ...
    let reportData = {
      ...
      sheet: 'Справка',
      colsizes: [0,5,18,15,30,40,10,40,15,20,15,20,15],
      header: ['No','Район','№ ОПОС','Имена','Адрес','Код','Уред','Брой'
              ,'Статус на уреда','№ поръчка','Договор','Статус на договора'],
      data: dataForExcel,
  ...
  back() {
    this.router.navigate(['/pages/register/regfilter/19/'+this.filter.descript])
  }
}
